"""FastAPI application for BracketDiffusion web service."""
...
async def lifespan(app: FastAPI):
    ...
    # Load BracketDiffusion pipeline
    from .inference import BracketDiffusionPipeline
    logger.info("Loading BracketDiffusion pipeline...")

    try:
        app.state.pipeline = BracketDiffusionPipeline()
...
app = FastAPI(title="BracketDiffusion Service", lifespan=lifespan)
...
async def generate(job_id: str, req: GenerateRequest):
    ...
    # Validate odd num_brackets
    if req.num_brackets % 2 == 0:
        raise HTTPException(400, "num_brackets must be odd")
...
async def hdr_raw(job_id: str, max_dim: int = Query(default=1024, ge=64, le=4096)):
    """Serve HDR data as raw float32 binary for client-side tone mapping.
