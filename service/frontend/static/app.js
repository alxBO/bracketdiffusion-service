...
// --- Slider labels ---
$('#brackets-slider').addEventListener('input', (e) => {
    $('#brackets-val').textContent = e.target.value;
});
$('#ev-slider').addEventListener('input', (e) => {
    $('#ev-val').textContent = e.target.value;
});
$('#steps-slider').addEventListener('input', (e) => {
    $('#steps-val').textContent = e.target.value;
});
$('#seed-input').addEventListener('input', (e) => {
    $('#seed-val').textContent = e.target.value;
});
$('#lambda0-slider').addEventListener('input', (e) => {
    $('#lambda0-val').textContent = parseFloat(e.target.value).toFixed(1);
});
$('#lambdas-slider').addEventListener('input', (e) => {
    $('#lambdas-val').textContent = parseFloat(e.target.value).toFixed(1);
});
$('#lambdad-slider').addEventListener('input', (e) => {
    $('#lambdad-val').textContent = parseFloat(e.target.value).toFixed(1);
});
$('#sigma-slider').addEventListener('input', (e) => {
    $('#sigma-val').textContent = parseFloat(e.target.value).toFixed(2);
});
...
$('#generate-btn').addEventListener('click', async () => {
    ...
    const params = {
        num_brackets: parseInt($('#brackets-slider').value),
        ev_steps: parseFloat($('#ev-slider').value),
        diffusion_steps: parseInt($('#steps-slider').value),
        seed: parseInt($('#seed-input').value),
        lambda_0: parseFloat($('#lambda0-slider').value),
        lambda_s: parseFloat($('#lambdas-slider').value),
        lambda_d: parseFloat($('#lambdad-slider').value),
        noise_sigma: parseFloat($('#sigma-slider').value),
        crf_type: $('#crf-select').value,
    };
